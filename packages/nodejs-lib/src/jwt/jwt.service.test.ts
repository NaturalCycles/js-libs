...
const noSchemaService = new JWTService({
  privateKey,
  publicKey: privateKey,
  algorithm: 'ES256',
})

test('jwtService all operations', async () => {
  const token1 = await jwtService.sign(data1, { expiresAt: null })
  ...
  const verified1 = await jwtService.verify(token1)
  expect(verified1).toStrictEqual(data1)

  // verify with explicit publicKey override
  const verified2 = await jwtService.verify(token1, { publicKey: privateKey })
...
test('expired token throws JWTExpiredError', async () => {
  const expiredToken = await jwtService.sign(data1, {
    expiresAt: localTime.now().minus(2, 'minute').unix,
  })

  const err = await pExpectedError(jwtService.verify(expiredToken), JWTExpiredError)
...
test('not-yet-valid token throws JWTNotYetValidError', async () => {
  const token1 = await jwtService.sign(data1, {
    expiresAt: null,
    notBefore: localTime.now().plus(1, 'hour').unix,
  })

  const err = await pExpectedError(jwtService.verify(token1), JWTNotYetValidError)
...
test('tryToVerify', async () => {
  ...
  const token1 = await jwtService.sign(data1, { expiresAt: null })
  expect(await jwtService.tryToVerify(token1)).toEqual([null, data1])
  ...
  expect(await jwtService.tryToVerify('yabadabadoo')).toEqual([expect.any(JWTInvalidError), null])
})

test('tryToVerifyOrDecode', async () => {
  // valid token: no error, trusted payload
  const token1 = await jwtService.sign(data1, { expiresAt: null })
  expect(await jwtService.tryToVerifyOrDecode(token1)).toEqual([null, data1])
  ...
  const [err4, payload4] = await jwtService.tryToVerifyOrDecode(tamperedToken)
  expect(err4).toBeInstanceOf(JWTInvalidError)
  expect(payload4).toStrictEqual(data1)

  // undecodable garbage: error + null
  const [err5, payload5] = await jwtService.tryToVerifyOrDecode('yabadabadoo')
...
test('verifyOptions: now and clockTolerance', async () => {
  ...
  const expiredToken = await jwtService.sign(data1, { expiresAt })

  // verify "from the past", when the token was not expired yet
  const verified = await jwtService.verify(expiredToken, {
  ...
  const verified2 = await jwtService.verify(expiredToken, {
    clockTolerance: 180,
  })
  expect(verified2).toMatchObject(data1)

  // without tolerance it still fails
  await pExpectedError(jwtService.verify(expiredToken), JWTExpiredError)
})
...
test('malformed and tampered tokens throw JWTInvalidError', async () => {
  ...
  const err1 = await pExpectedError(jwtService.verify(malformedToken), JWTInvalidError)
  expect(err1.data.code).toBe('JWT_INVALID')

  await pExpectedError(jwtService.verify(tamperedToken), JWTInvalidError)

  expect(() => jwtService.decode(malformedToken)).toThrowErrorMatchingInlineSnapshot(
  ...
  const decoded = jwtService.decode(tamperedToken)
...
test('schema: cfg-level, opt-level override, claim stripping', async () => {
  ...
  const err1 = await pExpectedError(jwtService.sign(invalidData, { expiresAt: null }))
  ...
  const err3 = await pExpectedError(jwtService.verify(fooToken))
  ...
  const service401 = new JWTService({
    privateKey,
    publicKey: privateKey,
    algorithm: 'ES256',
    schema: dataSchema,
  ...
  const verified = await jwtService.verify(token1)
...
test('verifyAlgorithms: one verifier accepting multiple key types', async () => {
  ...
  const rsaSigner = new JWTService({ privateKey: rsaPrivatePem, algorithm: 'RS256' })
  ...
  const multiVerifier = new JWTService({
